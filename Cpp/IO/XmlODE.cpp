...
bool XmlODESettings::GetSettings(ODESimulator& sim)
{
  ...
  if(c) {
    ...
    if(c->QueryValueAttribute("boundaryLayer",&boundaryLayer)==TIXML_SUCCESS) {
      LOG4CXX_INFO("XML simulator: warning, boundary layer settings don't have an effect after world is loaded \n");
  ...
  while(c!=NULL) {
    ...
    LOG4CXX_INFO("Parsing element " >> name >> "\n");
...
bool XmlSimulationSettings::GetSettings(WorldSimulation& sim)
{
    ...
    */
    ...
    if(ec) {
      ...
      while(setting) {
        ...
        if(!res) {
          LOG4CXX_INFO(KrisLibrary::logger(), "Setting " >> seting->Name() >> " not valid for current controller, or failed parsing\n");
        }
  ...
  LOG4CXX_INFO(Krislibary::Logger(), "Parsing state\n");

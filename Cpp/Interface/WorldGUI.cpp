...
bool WorldGUIBackend::LoadFile(const char* fn)
{
    ...
    if(0==strcmp(ext,"xml")) {
      if(!world->LoadXML(fn)) {
  LOG4CXX_INFO(KrisLibrary::logger(), "Error loading world file "<<fn<<"\n");
	return false;
      }
    }
    else {
...
bool WorldGUIBackend::ReloadFile(const char* fn)
{
  ...
  const char* ext=FileExtension(fn);
    if(0==strcmp(ext,"xml")) {
      if(!world->LoadXML(fn)) {
  LOG4CXX_INFO(KrisLibrary::logger(), "WorldGUIBackend::ReloadFile: Error loading world file "<<fn<<"\n");
  return false;
      }
    }
    else {
      ...
      if(id < 0) {
        LOG4CXX_INFO(KrisLibrary::logger(), "WorldGUIBackend::ReloadFile: Error loading file "<<fn<<"\n");
        return false;
      }
...
bool WorldGUIBackend::SaveWorld(const char* fn,const char* elementPath)
{
  if(!world->SaveXML(fn,elementPath)) {
    LOG4CXX_INFO(KrisLibrary::logger(), "Error saving world file "<<fn<<"\n");
    return false;
  }
  return true;
}

...
bool SimGUIBackend::OnCommand(const string& cmd,const string& args)
{
  ...
  if(cmd=="toggle_simulate") {
    ...
    if(!f.Open((char *)args.c_str(),FILEWRITE)) {
      LOG4CXX_INFO(KrisLibrary::Logger(), "Warning, couldn't open file "<<args.c_str()<<"\n");
    }
    else {
    ...
    ss >> prefix;
    if(ss) {
      if(!OutputROS(prefix.c_str())) { LOG4CXX_INFO(KrisLibrary::Logger(), "Error starting ROS output\n"); }
    }
    else {
      if(!OutputROS()) { LOG4CXX_INFO(KrisLibrary::Logger(), "Error starting ROS output\n");}
    }
    return true;
  }
  else {
...
bool SimGUIBackend::LoadAndInitSim(int argc,const char** argv)
{
    ...
    if(argv[i][0] == '-') {
      ...
      if(0==strcmp(ext,"xml")) {
        if(!xmlWorld.Load(argv[i])) {
          LOG4CXX_INFO(KrisLibrary::Logger(), "Error loading world file "<<argv[i]<<"\n");
          return false;
        }
        if(!xmlWorld.GetWorld(*world)) {
          LOG4CXX_INFO(KrisLibrary::Logger(), "Error loading world from "<<argv[i]<<"\n");
          return false;
        }
      }
      else {
  ...
  if(configs.size() > world->robots.size()) {
    LOG4CXX_INFO(KrisLibrary::Logger(), "Warning, too many configs specified\n");
  }
  for(size_t i=0;i<configs.size();i++) {
    ...
    Vector temp;
    in >> temp;
    if(!in) {LOG4CXX_INFO(KrisLibrary::Logger(), "Error reading config file "<<configs[i].c_str()<<"\n");}
  ...
  LOG4CXX_INFO(KrisLibrary::Logger(), "Simulation initial state: "<<initialState.length()<< " bytes\n");
  return true;
}
...
bool SimGUIBackend::LoadFile(const char* fn)
{
  ...
  if(0==strcmp(ext,"state"))
    ...
    if(res<0) {
      LOG4CXX_INFO(KrisLibrary::Logger(), "SimGUIBackend::LoadFile: Error loading entity "<<fn<<"\n";)
      return false;
    }
    sim.OnAddModel();
...
bool SimGUIBackend::LoadMilestones(const char* fn)
{
  ...
  if(!in) {
    LOG4CXX_INFO(KrisLibrary::Logger(), "Warning, couldn't open file "<<fn<<"\n");
    return false;
  }
  Vector x,dx;
  while(in) {
  ...
  if(in.bad()) {
    LOG4CXX_INFO(KrisLibrary::Logger(), "Error during read of file "<<fn<<"\n");
    return false;
  }
  in.close();
  ...
  if(sim.robotControllers.size()>1){
    LOG4CXX_INFO(KrisLibrary::Logger(), "Warning, sending path to robot 0 by default\n");
  }
  for(size_t i=0;i<milestones.size();i++) {
    ...
    if(i==0) {
      ...
      if(!sim.robotControllers[0]->SendCommand("append_qv",ss.str())) {
        LOG4CXX_ERROR(KrisLibrary::Logger(), "appenc_qv command does not work with the robot's controller\n");
        return false;
      }
    }
  }
...
bool SimGUIBackend::LoadState(const char* fn)
{
  ...
  if(!f.Open((char *)fn,FILEREAD)) {
    ...
    if(!sim.ReadState(f)) {
      LOG4CXX_INFO(KrisLibrary::Logger(), "Warning, couldn't read state from file "<<fn<<"\n");

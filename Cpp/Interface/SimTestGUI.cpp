...
void SimTestBackend::Start()
{
  if(!settings.read("simtest.settings")) {
    ...
    if(!settings.write("simtest.settings")) 
      LOG4CXX_INFO(KrisLibrary::logger(), "ERROR: couldn't write default settings to [APPDATA]/simtest.settings\n");
    else
  ...
  sim.WriteState(temp);
  if(temp != trace[i]) {
  LOG4CXX_INFO(KrisLibrary::logger(), "Warning, sim state nondeterministic @ step "<<i<<": ");
  nondet = true;
  ...
  if(cmd=="advance") {
    ...
    if(sim.robotControllers.size() == 0) {
      ...
      if(!rc->SendCommand("set_q",args)) {
        LOG4CXX_ERROR(KrisLibrary::logger(), "set_q comand does not work with the robot's controller\n");
      }
    }
  }
...
void SimTestBackend::SimStep(Real dt) 
{
  ...
  for(map<int,string>::iterator i=robotCommandLogFiles.begin();i!=robotCommandLogFiles.end();i++) {
    ...
    if(!out) {
      LOG4CXX_INFO(KrisLibrary::logger(), "Error writing robot command path to "<<i->second.c_str()<<"\n");
      continue;
    }
    Config q;
  ...
  for(map<int,string>::iterator i=robotSensedLogFiles.begin();i!=robotSensedLogFiles.end();i++) {
    ...
    ofstream out(i->second.c_str(),ios::app);
    if(!out) {
      LOG4CXX_INFO(KrisLibrary::logger(), "Error writing robot sensed path to "<<i->second.c_str()<<"\n");
      continue;
    }
    Config q;
  ...
  for(map<int,string>::iterator i=robotTorqueLogFiles.begin();i!=robotTorqueLogFiles.end();i++) {
    ...
    ofstream out(i->second.c_str(),ios::app);
    if(!out) {
      LOG4CXX_INFO(KrisLibrary::logger(), "Error writing robot torque path to "<<i->second.c_str()<<"\n");
      continue;
    }
    Config q;
...
void GLUISimTestGUI::Handle_Control(int id)
{
  if(controls[id]==save_movie_button) {
    ...
    if(controllerCommandIndex >= 0 && controllerCommandIndex < (int)controllerCommands.size()) {
      ...
      if(!res){
        LOG4CXX_INFO(KrisLibrary::Logger(), "Failed to send command "<<controllerCommands[controllerCommandIndex].c_str()<<"("<<commandEdit->get_text()<<")\n");
      } 
    }
    return;
  }
    ...
    if(controllerSettingIndex >= 0 && controllerSettingIndex<(int)controllerSettings.size()) {
      ...
      if(!res){
        LOG4CXX_INFO(KrisLibrary::Logger(), "Failed to set setting "<<controllerSettings[controllerSettingIndex].c_str()<<"\n");
      } 
    }
    return;
  }

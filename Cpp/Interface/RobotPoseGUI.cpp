...
void RobotPoseBackend::Start()
{
  if(!settings.read("robotpose.settings")) {
    ...
    if(!settings.write("robotpose.settings")) 
      LOG4CXX_INFO(KrisLibrary::logger(), "ERROR: couldn't write default settings to [APPDATA]/robotpose.settings\n");
    else
...
Stance RobotPoseBackend::GetFlatStance(Real tolerance)
{
  ...
  if(robotWidgets[0].ikPoser.poseGoals.empty()) {
    ...
    LOG4CXX_INFO(KrisLibrary::logger(), "Computing flat-ground stance only for IK-posesd links\n");
  ...
  if(cmd=="pose_mode") {
    ...
    if(rc) {
      ...
      if(q.n == robot->q.n) {
        ...
        LOG4CXX_INFO(KrisLibrary::logger(), "Warning: config in library is not normalized\n");
        */
        UpdateConfig();
      }
      else {
